...
transfer_routes_ipv4() {
        ...
        done << $(ip -4 rou show table main | grep "dev ${iface}" | sed -r 's_expires [^ ]*__' | sed -r 's_proto [^ ]*__' )
}
...
transfer_routes_ipv6() {
        ...
        done << $(ip -6 route show table main | grep "dev ${iface}" | sed -r 's_expires [^ ]*__' | sed -r 's_proto [^ ]*__' )
}
